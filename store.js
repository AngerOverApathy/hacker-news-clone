...
function favoritesReducer (state = initialState, action) {  //state object is default parameter
    switch (action.type) {
        ...
        case "REMOVE_FAVORITE": {
            ...
            state.favorites.filter(favorite => favorite.id !== removedFavorite.id); //use id to remove item from array
...
//sample action
const action = { type: "ADD_FAVORITE", payload: { favorite: { title: "story1", id: 1 } } } //a payload provides data
...
console.log(store.getState());//gets updated state

export default store; //provides object to getState and dispatch